...
{
    script_buf = (char *)shmat(shmid, 0, 0);
    ...
    db_debug("script: main key count #%d", mainkey_cnt);
    db_msg("script: V%d.%d.%d", head->version[0],
...
int script_fetch(const char *main_name, const char *sub_name, int value[], int count)
{
    int i, j;
    struct script_mainkey *mainkey;
