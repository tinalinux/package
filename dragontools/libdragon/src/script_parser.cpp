...
static int __parse_script(char *buf, int len)
{
    ...
    db_debug("the length of script is %d", len);

    /* allocate memory for key and value */
    item_table = (struct script_item *)malloc(sizeof(struct script_item) * ITEM_MAX_COUNT);
    if (item_table == NULL) {
        db_debug("allocate memory for main key failed(%s)", strerror(errno));
        ret = -1;
        goto out;
    }
    memset(item_table, 0, sizeof(struct script_item) * ITEM_MAX_COUNT);

    key_data = (char *)malloc(512 * 1024);
    if (key_data == NULL) {
        db_debug("allocate memory for key data failed(%s)", strerror(errno));
        ret = -1;
        goto out;
    }
    ...
    value_data = (int *)malloc(512 * 1024);
    if (value_data == NULL) {
        ...
        switch (flag) {
            ...
            case LINE_SUBKEY:
                ...
                switch (ret) {
                    ...
                    default:
                        db_debug("%s: L%d: fix me", __func__, __LINE__);
                }
            ...
            default:
                db_debug("script format error at line %d", line_num);
                ret = -1;
                goto out;
        }
    ...
    if (mainkey_idx <= 0) {
        db_debug("mainkey_idx = %d", mainkey_idx);
        ret = -1;
        goto out;
    }
    ...
    db_debug("script shmid = %d", shmid);

    script_buf = pos = (char *)shmat(shmid, 0, 0);
...
int parse_script(const char *name)
{
    ...
    /* read script data */
    fscript = fopen(name, "r");
    if (fscript == NULL) {
        db_error("script: can't open %s(%s)", name, strerror(errno));
        return -1;
    }
    ...
    if (read_len < len) {
        db_debug("len = %d, read_len = %d, fix me", len, read_len);
    }
    fclose(fscript);
