...
int outlog_init(void)
{
    ...
    mjson_foreach(OUTLOG_PATH, key, val, mtype) {
        // check type
        if (mtype == mjson_type_boolean) {
            if (val.m_boolean == true)
                selected = 1;
            else
                continue;
        }
        else if (mtype == mjson_type_object) {
            snprintf(buf, sizeof(OUTLOG_PATH) + 100, "%s/%s/%s",
                    OUTLOG_PATH, key, SYSKEY_GLOBAL_INFO_OUTLOG_ENABLE);
            if (mjson_fetch_boolean(buf) == true)
                selected = 1;
            else
                continue;
        }
        ...
        if (selected == 1) {
            ...
            char *p = strchr(key, '_');
            if (p == NULL) {
                ERROR("%s: can't spilt for module name\n", key);
                continue;
            }
            p++;
            snprintf(buf, sizeof(OUTLOG_PATH) + 100, "%s/outlog_%s.so", MODULE_LIB_PATH, p);
