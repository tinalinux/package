...
#define BASE_JSON_PATH "/global/info/outlog/outlog_markdown/outdir"
...
static inline void md_out_attr_attribute(struct task *task, FILE *fp)
{
    ...
    fprintf(fp, "| limit | run_time_sec | %d |\n",
            task->env.limit.run_time_sec);
    fprintf(fp, "\n");
}
