...
workers=`script_fetch "dram" "workers"`
dram_size=`script_fetch "dram" "dram_size"`
test_size=`script_fetch "dram" "test_size"`

outlog=`script_fetch "dram" "outlogname"`
errlog=`script_fetch "dram" "errlogname"`

actual_size=`get_dram_size`

if [ "x$platform" = "xa64" ] ; then
	dram_freq=`cat /sys/class/devfreq/dramfreq/cur_freq | awk -F: '{print $1}'`
elif [ "x$platform" = "xr40" ] ; then
	dram_freq=`cat /sys/devices/1c62000.dramfreq/devfreq/dramfreq/cur_freq | awk -F: '{print $1}'`
else
...
do
memtester $test_size"M" 1 1>/tmp/$outlog$i 2>/tmp/$errlog$i &
eval pid$i=$!
done
...
eval wait \$pid$i
exitstatus=$?
echo pid$i--exitstatus=$exitstatus
status=$(($status + $exitstatus))
cat /tmp/$outlog$i >> /tmp/$outlog
rm /tmp/$outlog$i
cat /tmp/$errlog$i >> /tmp/$errlog
rm /tmp/$errlog$i
done
