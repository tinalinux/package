...
static int init_layer(void)
{
	...
	if(ret) {
		db_error("######################ION_IOC_ALLOC err, ret %d, handle 0x%08x", ret, (unsigned int)alloc_data.handle);
		goto out1;
	}
	/* 3. get dmabuf fd */
	...
	if(ret) {
		db_error("##########################ION_IOC_MAP err, ret %d, dmabuf fd 0x%08x", ret, (unsigned int)fd_data.fd);
		goto out2;
	}
	/* 4. mmap to user space */
	buffer = mmap(NULL, alloc_data.len, PROT_READ|PROT_WRITE, MAP_SHARED, fd_data.fd, 0);
	if(MAP_FAILED == buffer) {
		db_error("#####################mmap err, ret %d", (unsigned int)buffer);
		goto out3;
	}
	...
	if(ret) {
		db_error("###############################ION_IOC_SUNXI_PHYS_ADDR err, ret %d", ret);
		goto out4;
	}
	db_msg("ION_IOC_SUNXI_PHYS_ADDR succes, phys_addr 0x%08x, size 0x%08x", phys_data.phys_addr, phys_data.size);
	...
	db_msg("########screen_width=%d",screen_width);
	db_msg("########screen_height=%d",screen_height);
...
out2:
	...
	if(ret)
		db_error("ION_IOC_FREE err, ret %d", ret);
out1:
	/* close ion device */
	close(fd);
	return ret;
...
static int detect_output_mode(void)
{
    ...
    if (ret == 1) {
        db_msg("hdmitester: your device support 1080p 50Hzn");
        ...
        if (ret == 1) {
            ...
            db_msg("hdmitester: use user config mode: %s", type_list[support_mode].name);
            args[0] = 1;
            args[1] = type_list[support_mode].mode;
            ret = ioctl(disp, DISP_HDMI_SUPPORT_MODE, args);
            if (ret == 1) {
                db_msg("hdmitester: you device support %s", type_list[support_mode].name);
...
static int xrun_recovery(snd_pcm_t *handle, int err)
{
    ...
    if (err < 0) {
        db_warn("Can't recovery from underrun, prepare failed: %s", snd_strerror(err));
    }
        ...
        while ((err = snd_pcm_resume(handle)) == -EAGAIN) {
            ...
            if (err < 0) {
                db_warn("Can't recovery from suspend, prepare failed: %s", snd_strerror(err));
            }
        }
...
static void *sound_play(void *args)
{
    ...
    db_msg("samplerate #%d", samplerate);

    err = snd_pcm_open(&playback_handle, "hw:1,0", SND_PCM_STREAM_PLAYBACK, 0);
    if (err < 0) {
        db_error("cannot open audio device (%s)", snd_strerror(err));
        pthread_exit((void *)-1);
    }

    err = snd_pcm_hw_params_malloc(&hw_params);
    if (err < 0) {
        db_error("cannot allocate hardware parameter structure (%s)", snd_strerror(err));
        pthread_exit((void *)-1);
    }

    err = snd_pcm_hw_params_any(playback_handle, hw_params);
    if (err < 0) {
        db_error("cannot initialize hardware parameter structure (%s)", snd_strerror(err));
        pthread_exit((void *)-1);
    }
    ...
    if (err < 0) {
        db_error("cannot set sample rate (%s)", snd_strerror(err));
        pthread_exit((void *)-1);
    }

    err = snd_pcm_hw_params_set_channels(playback_handle, hw_params, 2);
    if (err < 0) {
        db_error("cannot set channel count (%s), err = %d", snd_strerror(err), err);
        pthread_exit((void *)-1);
    }

    err = snd_pcm_hw_params(playback_handle, hw_params);
    if (err < 0) {
        db_error("cannot set parameters (%s)", snd_strerror(err));
        pthread_exit((void *)-1);
    }

    snd_pcm_hw_params_free(hw_params);

    db_msg("open test pcm file: %s", path);
    fp = fopen(path, "r");
    if (fp == NULL) {
        db_error("cannot open test pcm file(%s)", strerror(errno));
        pthread_exit((void *)-1);
    }
    ...
    while (1) {
        while (!feof(fp)) {
            ...
            if (err < 0) {
                db_warn("read test pcm failed(%s)", strerror(errno));
            }
            ...
            if (err < 0) {
                ...
                if (err < 0) {
                    db_warn("write error: %s", snd_strerror(err));
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    if (disp == -1) {
        db_error("hdmitester: open /dev/disp failed(%s)", strerror(errno));
        goto err;
    }
   ...
   /* test main loop */
    while (1) {
        ...
        sleep(1);
		fd_hdmi = open("/sys/class/switch/hdmi/state", O_RDONLY);
		if (fd_hdmi < 0){
			db_error("hdmitester: open /sys/class/switch/hdmi/state failed");
		}
        ...
        if (val == '1') {
             ...
             screen_height = 1080;
#endif
			 db_msg("hdmitester: read /sys/class/switch/hdmi/state 1");
		 args[1] = DISP_OUTPUT_TYPE_HDMI;
             args[2] = output_mode;
             ioctl(disp, DISP_DEVICE_SWITCH, (unsigned long)args);
