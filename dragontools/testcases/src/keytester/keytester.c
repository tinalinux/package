...
static int match_keyboard(char *dev_name, char *key_name)
{
    return strncmp(dev_name, key_name, strlen(key_name));
}
...
int main(int argc, char *argv[])
{
    ...
    if (script_fetch("key", "key_name", (int *)key_name, sizeof(key_name) / 4)) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        if (access(dev_path, F_OK)) {
            db_error("can't access %s(%s)", dev_path, strerror(errno));
            goto err;
        }

        fp = fopen(dev_path, "r");
        if (fp == NULL) {
            db_error("can't open %s(%s)", dev_path, strerror(errno));
            goto err;
        }

        if (fgets(evt_path, 32, fp) == NULL) {
            db_error("can't read %s(%s)", dev_path, strerror(errno));
            goto err;
        }

        if (match_keyboard(evt_path, key_name) == 0) {
            fclose(fp);
            break;
        }
        fclose(fp);
    ...
    if ((fd = open(evt_path, O_RDONLY)) < 0) {
        db_error("can't open %s(%s)", evt_path, strerror(errno));
        goto err;
    }
    ...
    db_debug("Input device name: \"%s\"", name);
    ...
    for (i = 0; i < EV_MAX; i++) {
        if (test_bit(i, bit[0])) {
            db_debug("    Event type %d (%s)", i, events[i] ? events[i] : "?");
            if (!i)
                continue;
            ...
            for (j = 0; j < KEY_MAX; j++) {
                if (test_bit(j, bit[i])) {
                    db_debug("        Event code %d (%s)", j, names[i] ? (names[i][j] ? names[i][j] : "?") : "?");
                    if (i == EV_ABS) {
                        ioctl(fd, EVIOCGABS(j), abs);
                        for (k = 0; k < 5; k++) {
                            if ((k < 3) || abs[k])
                                printf("%s %6d", absval[k], abs[k]);
                        }
                    }
                }
            }
    ...
    if (script_fetch("mic", "key_enable", &key_enable, 1) == 0 &&
            key_enable == 1) {
        db_debug("mkfifo %s", VOLUME_PIPE_NAME);
        unlink(VOLUME_PIPE_NAME);
        if (mkfifo(VOLUME_PIPE_NAME, S_IFIFO | 0666) == -1) {
            db_error("mkfifo %s error(%s)", VOLUME_PIPE_NAME, strerror(errno));
        }
        else {
            volume_pipe = fopen(VOLUME_PIPE_NAME, "w");
            if (volume_pipe == NULL) {
                db_error("can't open %s(%s)", VOLUME_PIPE_NAME, strerror(errno));
            }
            else {
            ...
            if (evt[i].type == EV_SYN) {
                ...
                if (volume_pipe) {
                    ...
                    if (strncmp(volume_key, "VolumeDown", 10) == 0 ||
                        ...
                        fprintf(volume_pipe, "%s", volume_key);
                    }
                }
            }
        }
