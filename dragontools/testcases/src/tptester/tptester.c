...
int main(int argc, char *argv[])
{
    ...
    if(script_fetch("tp", "tp_name", (int *)tp_name, sizeof(tp_name) / 4)){
        strcpy(tp_name, "ft5x_ts");
    }

    db_msg("tp_name:%s", tp_name);
    ...
    for (i = 0; i < 6; i++) {
        ...
        db_msg("tp_name:%s, dev_name:%s", tp_name, evt_path);
    ...
    if (i == 6) {
        db_error("NO!!! touchscreen input found");
        goto err;
    }
    ...
    while (1) {
        ...
        for (i = 0; i < rd / sizeof(struct input_event); i++) {
            if (evt[i].type == EV_SYN) {
                ...
                if (evt[i].code == 0x36) {
                    abs_y = evt[i].value;
                    snprintf(buf, sizeof(buf), "ABS(%d, %d)", abs_x, abs_y);
                    SEND_CMD_PIPE_OK_EX(buf);
                }
