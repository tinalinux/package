...
{
    db_error("%s", dev_name);
    return strncmp(dev_name, "sunxi_ir", strlen("sunxi_ir"));
}
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < EV_MAX; i++) {
        if (test_bit(i, bit[0])) {
            db_dump("    Event type %d (%s)", i, events[i] ? events[i] : "?");
            if (!i)
                continue;
            ...
            for (j = 0; j < KEY_MAX; j++) {
                if (test_bit(j, bit[i])) {
                    db_dump("        Event code %d (%s)", j, names[i] ? (names[i][j] ? names[i][j] : "?") : "?");
                    if (i == EV_ABS) {
