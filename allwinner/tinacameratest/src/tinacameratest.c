...
{
	hawkview->detect.dev_id		= atoi(argv[7]);
...
void hawkview_start(hawkview_handle *hv)
{
	if (strcmp(hv->detect.media_type,"photo") == 0) {
		...
		init_capture(hv);
		init_encoder(hv);
		if (hv->detect.camera_type == usb_camera) {
		...
		hv->encoder.ENCODE_FMT = VIDEO_ENCODE_H264;
...
int main(int argc, char *argv[])
{
	...
	hawkview->encoder.flag_UsePhyBuf = 1;
