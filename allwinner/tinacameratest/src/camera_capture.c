...
int init_capture(void *arg)
{
	...
	for (n_buffers= 0; n_buffers < req.count; ++n_buffers) {
		...
		hv->capture.buffers[n_buffers].start = mmap(NULL , buf.length,
										...
										buf.m.offset);
		hv_msg("map buffer index: %d, mem: %p, len: %x, offset: %lu\n",
				n_buffers, hv->capture.buffers[n_buffers].start, buf.length, buf.m.offset);
	}
...
int capture_quit(void *arg)
{
	...
	for (i = 0; i < hv->capture.buf_count; i++) {
		hv_msg("ummap index: %d, mem: %p, len: %x\n",
				i, hv->capture.buffers[i].start, (unsigned int)hv->capture.buffers[i].length);
