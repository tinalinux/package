...
int disp_video_chanel_open(void)
{
	...
	if (0 == disp_video_start_flag) {
		...
		for(screen_id = 0; screen_id < 3; screen_id++) {
			ioctlParam[0] = screen_id;
			ret = ioctl(disp_fd, DISP_GET_OUTPUT_TYPE, (void*)ioctlParam);
		}
...
#else
	printf("m:%d pipe:%d alen:%d alval:%d src:(%d %d, %dx%d) scn(%d %d, %dx%d)\n",
...
int disp_video_layer_param_set(unsigned int phy_addr[],
		enum VIDEO_PIXEL_FORMAT fmt, struct win_info *win, unsigned int layer_id)
{
#ifdef __SUNXI_DISPLAY2__
	disp_layer_config config;
#else
	...
	DISP_CLAR(config);
	config.channel = (layer_id == 1) ? 0:1;
    config.layer_id = 0;
	ret = disp_layer_param_get(disp_fd, layer_id, (void *)&config);
    ...
    switch(fmt)
    {
        ...
        config.info.fb.size[0].height	= win->src_win.nHeight;
        config.info.fb.size[1].width = win->src_win.nWidth/2;
        config.info.fb.size[1].height = win->src_win.nHeight/2;
        config.info.fb.size[2].width = win->src_win.nWidth/2;
        config.info.fb.size[2].height = win->src_win.nHeight/2;
        break;
    ...
    config.info.zorder     	= (layer_id == 1) ? 3:4;
    config.info.alpha_mode 	= 1;
    config.info.alpha_value 	= 0xff;
	config.info.fb.scan= DISP_SCAN_PROGRESSIVE;

    //* image size.
    config.info.fb.crop.x = (unsigned long long)win->src_win.crop_x<< 32;
    config.info.fb.crop.y = (unsigned long long)win->src_win.crop_y<< 32;
    config.info.fb.crop.width   = (unsigned long long)win->src_win.crop_w << 32;
    config.info.fb.crop.height  = (unsigned long long)win->src_win.crop_h << 32;
    config.info.fb.color_space  = (win->src_win.nHeight < 720) ? DISP_BT601 : DISP_BT709;

    //* source window.
    config.info.screen_win.x        = win->scn_win.x;
    config.info.screen_win.y        = win->scn_win.y;
    config.info.screen_win.width    = win->scn_win.w;
    config.info.screen_win.height   = win->scn_win.h;
    config.enable = 1;
#else
    switch(fmt)
    {
	...
	config.screen_win.x 	 =  win->scn_win.x;
	config.screen_win.y 	 =  win->scn_win.y;
	config.screen_win.width  = win->scn_win.w;
	config.screen_win.height	 =  win->scn_win.h;

#endif
	ret = disp_layer_param_set(disp_fd, layer_id, (void *)&config);
	if (ret < 0) {
		DISP_DBG_LOG("%s: fail to get layer param, fd=%d, layer=%u\n",
			__func__, disp_fd, disp_layer_id);
	}
