...
/* YUV 4:2:2 */
		...
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YUV422SP;
		return 0;
	} else if (strcmp(hv->detect.req_fmt, "NV61") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_NV61;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YVU422SP;
		return 0;
	}
/* YUYV YVYU UYVY VYUY */
	else if (strcmp(hv->detect.req_fmt, "YUYV") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_YUYV;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YUYV422;
		return 0;
	} else if (strcmp(hv->detect.req_fmt, "YVYU") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_YVYU;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YVYU422;
		return 0;
	} else if (strcmp(hv->detect.req_fmt, "UYVY") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_UYVY;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_UYVY422;
		return 0;
	} else if (strcmp(hv->detect.req_fmt, "VYUY") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_VYUY;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_VYUY422;
		return 0;
	}
/* H264 MJPEG */
	else if (strcmp(hv->detect.req_fmt, "H.264") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_H264;
		hv->decoder.flag_decoder = 1;
		hv->decoder.SOURCE_FMT = VIDEO_CODEC_FORMAT_H264;
		hv->decoder.DECODE_FMT = PIXEL_FORMAT_NV21;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YVU420SP;
		return 0;
	} else if (strcmp(hv->detect.req_fmt, "MJPEG") == 0) {
		hv->capture.cap_fmt = V4L2_PIX_FMT_MJPEG;
		hv->decoder.flag_decoder = 1;
		hv->decoder.SOURCE_FMT = VIDEO_CODEC_FORMAT_MJPEG;
		hv->decoder.DECODE_FMT = PIXEL_FORMAT_NV21;
		hv->encoder.SOURCE_FMT = VENC_PIXEL_YVU420SP;
		return 0;
	} else {
