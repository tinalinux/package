...
static void remove_watch(DBusWatch *watch, void *data)
{
    ...
    TLOGI("remove watch %p/n", (void*)watch);
    for(i=0; i<=max_i;i++) {
        if(watches[i] == watch) {
            found = 1;
            break;
	...
	switch(func_id) {
		...
		case NATIVEPOWER_DEAMON_ACQUIRE_WAKELOCK:
			if(DBUS_TYPE_STRING == dbus_message_iter_get_arg_type(&args)) {
				...
				ret = acquireWakeLock(PARTIAL_WAKE_LOCK, id);
			}
			break;
		...
		case NATIVEPOWER_DEAMON_SET_SCENE:
			if(DBUS_TYPE_UINT32 == dbus_message_iter_get_arg_type(&args)) {
				...
				if (reason >= 0 && reason < NATIVEPOWER_SCENE_MAX)
					ret = np_scene_change(str_scene[reason]);
			}
			break;
		default:
			TLOGE("invalid func id\n");
			break;
	}
...
void *dbus_server_thread_func(void *arg)
{
	...
	dbus_bus_request_name(conn, "nativepower.dbus.server", DBUS_NAME_FLAG_REPLACE_EXISTING, &err);
