...
int np_uci_read_config(NP_UCI *uci, const char *conf_section, const char *conf_name, char *conf_buf)
{
    int ret = -1;
...
int np_uci_write_config(NP_UCI *uci, const char *conf_section, const char *conf_name, const char *conf_buff)
{
    int ret = -1;
    struct uci_ptr ptr =
    { .package = "config", .section = conf_section, .option = conf_name, .value = conf_buff, };
    ...
    ret = uci_set(uci->context, &ptr); //写入配置
    if (UCI_OK == ret)
    {

        ret = uci_commit(uci->context, &ptr.p, true); //提交保存更改
    }
