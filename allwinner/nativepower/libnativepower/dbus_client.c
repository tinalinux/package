...
int dbus_client_invoke(DBusConnection *conn, const NativePower_Function func_id, const char *data, const int reason)
{
	DBusMessage *msg;
    DBusMessageIter arg;
    dbus_uint32_t value;
...
*/
	msg = dbus_message_new_method_call("nativepower.dbus.server",
                                        "/nativepower/service/method",
                                        "nativepower.method.interface",
                                        "method");
    if(!msg) {
	...
	if(!dbus_message_iter_append_basic(&arg, DBUS_TYPE_UINT32, (void *)&reason)) {
        fprintf(stderr, "out of memeory\n");
        dbus_message_unref(msg);
        return -1;
    }
...
int dbus_client_send_signal(DBusConnection *conn, const NativePower_Function func_id, const char *data, const int reason)
{
	DBusMessage *msg;
    DBusMessageIter arg;
    dbus_uint32_t value;
	...
	msg = dbus_message_new_signal("/nativepower/service/signal",
                                    "nativepower.signal.interface",
                                    "invoke");
    if(!msg) {
