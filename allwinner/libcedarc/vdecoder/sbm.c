...
Sbm *SbmCreate(int nBufferSize, struct ScMemOpsS *memops)
{
    ...
    pSbmBuf = (char*)CdcMemPalloc(memops, nBufferSize);    //*
    if(pSbmBuf == NULL)
    {
        loge("pSbmBuf == NULL.");
        return NULL;
...
// only for jpeg tile mode create sbm buffer
Sbm *SbmVirCreate(int nBufferSize)
{
    ...
    pSbmBuf = (char*)malloc(nBufferSize);    //*
    if(pSbmBuf == NULL)
    {
        loge("pSbmBuf == NULL.");
        return NULL;
...
void SbmDestroy(Sbm *pSbm)
{
    if(pSbm != NULL)
    {
        ...
        {
            CdcMemPfree(pSbm->memops,pSbm->pStreamBuffer);
