...
void *HwSecureAllocPalloc(int size)
{
    logv("SecureMemAdapterAlloc, size %d", size);
    if(size <= 0) {
        return NULL;
    }
    return CdcMemPallocSecure(gAdapterContext->ionMemOps, size);
}

void HwSecureAllocPfree(void *ptr)
{
    ...
    CdcMemPfree(gAdapterContext->ionMemOps, ptr);
}
...
void *SwSecureAllocPalloc(int size)
{
    logv("SwSecureAllocPalloc");
    return CdcMemPalloc(gAdapterContext->ionMemOps, size);
}

void SwSecureAllocPfree(void *ptr)
{
