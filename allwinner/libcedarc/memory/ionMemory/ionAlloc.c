...
void* ion_alloc_palloc(int size)
{
...
void ion_alloc_pfree(void * pbuf)
{
    int flag = 0;
...
{
    return (void*)((unsigned long)ion_alloc_vir2phy_cpu(pbuf) - VeGetPhyOffset());
}

void* ion_alloc_phy2vir_ve(void * pbuf)
{
    return (void*)((unsigned long)ion_alloc_phy2vir_cpu(pbuf) - VeGetPhyOffset());
}
...
void* ion_alloc_alloc_drm(int size)
{
    aw_ion_allocation_info_t alloc_data;
    ion_fd_data_t fd_data;
    struct ion_handle_data handle_data;
