...
void saveVideoPictureInfo(VideoPicture *p, char *file)
{
    static FILE *fp = NULL;
...
void *displayPictureThreadFunc(void* param)
{
    ...
    while(1)
    {
        ...
        if(pPicture != NULL)
        {
            ...
            if ( pVideoDecoder->pSaveShaFile!= NULL && strcmp(pVideoDecoder->pSaveShaFile,"") != 0)
            {
                saveVideoPictureInfo(pPicture,pVideoDecoder->pSaveShaFile);
            }
