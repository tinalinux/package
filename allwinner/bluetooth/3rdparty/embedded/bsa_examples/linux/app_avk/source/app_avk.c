...
static void avk_pcm_set(void)
{
	system("amixer cset numid=35 0");
	system("amixer cset numid=33 0");
	system("amixer cset numid=34 0");
	system("amixer cset numid=28 0");
	system("amixer cset numid=27 0");
	system("amixer cset numid=19 0");
}
    ...
    if (connection->format == BSA_AVK_CODEC_PCM)
    {
        ...
        else
        {
			...
			if (DEFAULT_OUT_SAMPLING_RATE != connection->sample_rate) {
				...
				ret = create_resampler(connection->sample_rate, DEFAULT_OUT_SAMPLING_RATE,\
						connection->num_channel, RESAMPLER_QUALITY_DEFAULT, NULL, &out.resampler);
				if (ret != 0) {
