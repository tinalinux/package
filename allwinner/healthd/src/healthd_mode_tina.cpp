...
static int dbus_service_send_signal(const char *member, int type, void *data)
{
	...
	//printf("send signal:member=%s, type=%d, data=%u\n", member, type, value);
	msg = dbus_message_new_signal("/healthd/service/signal",
									"healthd.signal.interface",
									member);
	if(!msg) {
...
void healthd_mode_tina_battery_update(struct BatteryProperties *props)
{
	...
	DLOG("before tina ops update:ac-%u,usb-%u,bat-%u,bat_status-%u,bat_health-%u,bat_cap-%u,bat_vol-%u,bat_temp-%u\n", props->chargerAcOnline, props->chargerUsbOnline, props->batteryPresent, props->batteryStatus, props->batteryHealth, props->batteryLevel, props->batteryVoltage, props->batteryTemperature);

	props->batteryLowCapWarn = false;
	if(props->batteryPresent && !props->chargerAcOnline && !props->chargerUsbOnline) {
		/*low capacity handle*/
		low_capacity_shutdown = batinfo_get_bat_capacity_alert_level2();
		if(low_capacity_shutdown >= 0 && props->batteryLevel <= low_capacity_shutdown) {
			/*low capacity shutdown*/
			//system("echo low_capacity_shutdown >> /root/shutdown_log");
			perform_shutdown(props);
		}
		low_capacity_warning = batinfo_get_bat_capacity_alert_level1();
		if(low_capacity_warning >= 0 && props->batteryLevel <= low_capacity_warning) {
	...
	if(props->batteryPresent) {
		...
		if(props->batteryTemperature != -0xFF &&
			...
			//system("echo bat_temp_low >> /root/shutdown_log");
			perform_shutdown(props);
		}
		if(props->batteryTemperature != -0xFF &&
			get_healthd_config("bat_temp_over", &bat_temp_over) &&
			props->batteryTemperature >=  bat_temp_over) {
			//system("echo bat_temp_over >> /root/shutdown_log");
			perform_shutdown(props);
		}
	}
	...
	if(ic_temp >= 0) {
		if(get_healthd_config("ic_high_temp_shutdown", &ic_high_temp_shutdown) && ic_temp >= ic_high_temp_shutdown) {
			/*high ic temp shutdown*/
			//system("echo ic_high_temp_shutdown >> /root/shutdown_log");
			perform_shutdown(props);
		}
		if(get_healthd_config("ic_high_temp_warning", &ic_high_temp_warning) && ic_temp >= ic_high_temp_warning) {
