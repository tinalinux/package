...
extern void healthd_mode_tina_battery_update(
...
/*
 void healthd_mode_tina_init(struct healthd_config* ) {
 ...
 void healthd_mode_tina_battery_update(struct BatteryProperties* ) {
...
static void wakealarm_set_interval(int interval){
    ...
    if (timerfd_settime(wakealarm_fd, 0, &itval, NULL) == -1)
       printf("wakealarm_set_interval: timerfd_settime failed\n");
}
...
static void wakealarm_init(void) {
	...
	if (wakealarm_fd == -1) {
		printf("wakealarm_init: timerfd_create failed\n");
		return;
	}

    if (healthd_register_event(wakealarm_fd, wakealarm_event))
		printf( "Registration of wakealarm event failed\n");
...
int main(int argc, char **argv) {
	...
	printf("healthd for test 19:52\n");


	/*
	if(is_charger)
	{
		is_charger_tmp = is_charger;
		printf("Tina powerdown charging\n");
...
/*
	if(is_charger_tmp){
		printf("healthd_mainloop is_charger_tmp=%d\n",is_charger_tmp);
		is_charger_tmp = 0;
		ret=set_power_state_mem();
	}
*/
    healthd_mainloop();
	printf("Main loop terminated, exiting\n");
    return 0;
}
