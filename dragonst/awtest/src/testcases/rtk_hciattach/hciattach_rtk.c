...
#define FIRMWARE_DIRECTORY_OLD "/system/etc/"
#define BT_CONFIG_DIRECTORY_OLD "/system/etc/"

//#define FIRMWARE_DIRECTORY "/system/etc/firmware/rtlbt/"
//#define BT_CONFIG_DIRECTORY "/system/etc/firmware/rtlbt/"
#define FIRMWARE_DIRECTORY "/system/etc/"
#define BT_CONFIG_DIRECTORY "/system/etc/"
...
static const char *get_firmware_name()
{
	...
	if (stat(firmware_file_name, &st) < 0)
	{
		...
		sprintf(firmware_file_name, FIRMWARE_DIRECTORY_OLD"rtl8723b_fw");
	}
...
*
*/
static void rtk_get_ram_addr(char bt_addr[4])
{
...
GET_CONFIG:
	...
	sprintf(bt_config_file_name, BT_CONFIG_DIRECTORY"rtl8723b_config");
	if (stat(bt_config_file_name, &st) < 0)
	{
		...
		sprintf(bt_config_file_name, BT_CONFIG_DIRECTORY_OLD"rtl8723b_config");
...
static void rtk_download_fw_config(int fd, RT_U8* buf, size_t filesize, int is_sent_changerate, int proto,struct termios *ti)
{
    ...
    if (is_sent_changerate && (gRom_version_cmd_state >= cmd_has_sent))
		iAdditionPkt = (iEndIndex+3)%8?(8-(iEndIndex+3)%8):0;
	else if(is_sent_changerate || (gRom_version_cmd_state >= cmd_has_sent))
...
static int rtk_config(int fd, int proto, int speed, struct termios *ti)
{
	...
	*/
         buf_len = rtk_get_bt_firmware(&epatch_buf, config_len);
			...
			RS_DBG("gLmpVersion=%d", lmp_version);
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									RS_DBG("fm_version = 0x%x",(unsigned int)epatch_info->fm_version);
									...
									RS_DBG("patch_length = 0x%x",(unsigned int)current_entry.patch_length);
									RS_DBG("start_offset = 0x%x",(unsigned int)current_entry.start_offset);
